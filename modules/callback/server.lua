...
RegisterNetEvent("Index.callback:Trigger", function(name, key, ...)
    ...
    if cb then
        ...
        TriggerClientEvent("Index.callback:Return", id, key, result)
    else
...
function module.use(name, plyId, ...)
    ...
    TriggerClientEvent("Index.callback:Trigger", plyId, name, key, table.unpack(data))
